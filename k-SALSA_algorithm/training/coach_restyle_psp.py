...
import tqdm
# from training.spherical_kmeans import MiniBatchSphericalKMeans as sKmeans
...
class Coach:
	def __init__(self, opts):
		...
		# Initialize dataset
		self.train_dataset, self.valid_dataset, self.test_dataset = self.configure_datasets()
	...
	def perform_train_iteration_on_batch(self, x, y):
		...
		for iter in range(self.opts.n_iters_per_batch):
			if iter == 0:
				...
				# print('x:',x.shape) #[8,3,256,256]
				# print('avg_image_for_batch:',avg_image_for_batch.shape) #[8,3,256,256]
				x_input = torch.cat([x, avg_image_for_batch], dim=1)
				# print('x_input_iter0:',x_input.shape) # [8,6,256,256]
				...
				# print('x:',x.shape) # [8,3,256,256]
				# print('y_hat_clone:',y_hat_clone.shape) # [8,3,256,256]
				x_input = torch.cat([x, y_hat_clone], dim=1)
				# print('x_input, latent_clone:',latent_clone, latent_clone)
				# print('x_input:',x_input.shape) # [8,6,256,256]
				y_hat, latent = self.net.forward(x_input, latent=latent_clone, return_latents=True)
			# print('self.net:',next(self.net.parameters()).device) # 'cuda:0'
			if self.opts.dataset_type == "cars_encode":
				y_hat = y_hat[:, :, 32:224, :]
	...
	def configure_datasets(self):
		...
		train_dataset = ImagesDataset(source_root=dataset_args['train_source_root'],
									  ...
									  opts=self.opts)
		valid_dataset = ImagesDataset(source_root=dataset_args['train_source_root'],
									 target_root=dataset_args['train_target_root'],
									 source_transform=transforms_dict['transform_source'],
