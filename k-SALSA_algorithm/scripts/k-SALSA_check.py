...
def project(
    ...
    *,
    ...
    for step in range(opts.num_steps):
        ...
        # < Content >
        y_hat_synth = net.face_pool(y_hat_synth)
        # print('y_hat_synth:',y_hat_synth.shape) # [1,3,256,256]
        y_hat_synth2 = y_hat_synth.repeat([source_images.shape[0],1,1,1]) #[1,3,256,256] -> [k,3,256,256]
        # print('y_hat_synth2:',y_hat_synth2.shape) # [5,3,256,256]
        ...
        centroid_gram = torch.zeros(opts.num_crop*opts.num_crop, channel_size, channel_size).to(device) # [16, 64, 64]
        source_gram = torch.zeros(len(source_images), opts.num_crop*opts.num_crop, channel_size, channel_size).to(device) # [k, 16, 64, 64]
        
        for idx in range(len(source_images)):
            t=0
            source_feature = torch.unsqueeze(source_features[style_layers][idx],0)
            # print('source_feature:',source_feature.shape) # [1,64,256,256]
            feature_size = source_feature.size(2) # 256
            crop_size = feature_size // opts.num_crop # 256//4 = 64?
            # print('crop_size:',crop_size) # 64

            for i in range(opts.num_crop):
                for j in range(opts.num_crop):
                    new_source_feat = T.functional.crop(source_feature, top=i*crop_size, left=j*crop_size, height=crop_size, width=crop_size)
                    # print('new_source_feat:',new_source_feat.shape) # [1,64,64,64]
                    new_source_style = gram_matrix(new_source_feat)
                    # print('new_source_style:',new_source_style.shape) # [64,64]
                    source_gram[idx][t] = new_source_style
                    t = t+1
        ...
        source_gram = source_gram.view(source_gram.size(0), source_gram.size(1), -1) # for similar image(target)
        ...
        for idx in range(len(source_images)):
            pos_idxes = []
            style_loss = 0
            for i in range(source_gram.size(1)):
                cos_output = cos(centroid_gram, torch.unsqueeze(source_gram[idx][i],0)) # 1 target, whole centroid
                pos_idx = cos_output.argmax(dim=-1)
                pos_idxes.append(pos_idx)
            for ix in range(len(pos_idxes)):
                style_loss += ((centroid_gram[pos_idxes[ix]] - source_gram[idx][ix].detach())**2).sum() 
