...
#             pdb.set_trace()
            ...
            ##############################
            y = F.one_hot(y.long(), num_classes=5).float()
            # print('y:',y)
            y0 = y.transpose(0,1)[0]
            y_rest = sum(y.transpose(0,1)[1:],0) 
            y = torch.stack([y0, y_rest]) 
            y = F.softmax(y.transpose(0,1))
#             pdb.set_trace()
            # forward
            y_pred = model(X)
            ##############################
...
#             pdb.set_trace()
            ...
            # print('y_pred:',y_pred.shape) # [32,1]
            # print('y:',y.shape) # [32]
...
def evaluate(cfg, model, checkpoint, test_dataset, estimator):
    ...
    return estimator.get_accuracy(6), estimator.get_kappa(6), estimator.conf_mat
...
def initialize_loss(cfg, train_dataset):
    ...
    elif criterion in  ['mean_square_error', 'mean_square_error2']:
