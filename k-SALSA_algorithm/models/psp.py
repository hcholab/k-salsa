...
class pSp(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.D = Discriminator(256, channel_multiplier=2)
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None: # pSp/restyle checkpoint
            ...
            self.D.load_state_dict(ckpt['d'], strict=True)
            self.decoder.load_state_dict(ckpt['g_ema'], strict=True)
            self.__load_latent_avg(ckpt, repeat=self.n_styles)

    def forward(self, x, batch_centroids=None, nearest_neighbor_centroid=None, computing_features=False, latent=None, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
                inject_latent=None, return_latents=False, alpha=None, average_code=False, input_is_full=False):
        # print('x in:',x.shape) # [8,6,256,256]
        if computing_features:
            if input_code:
                # print('input_code?',x.shape) # iter==0 -> [1,16,512]
                codes = x
            else:
            ...
            # print('self.decoder:',next(self.decoder.parameters()).device) # 'cuda:0'
            self.decoder = self.decoder.to(self.opts.device)
            # print('codes:',codes.shape) # [1,16,512](iter==0) -> [8,16,512]
        ...
        elif nearest_neighbor_centroid is not None:
            codes = nearest_neighbor_centroid.view(nearest_neighbor_centroid.shape[0],16,512)
            if average_code:
                input_is_latent = True
            else:
                input_is_latent = (not input_code) or (input_is_full)
            ...
            codes = batch_centroids.view(batch_centroids.shape[0],16,512)
            # print('x:',x[0])
            # print('batch_centroids:',batch_centroids[0])
